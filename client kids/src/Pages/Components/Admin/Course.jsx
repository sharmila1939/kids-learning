...
const Course = () => {
  ...
  useEffect(() => {
    const fetchCourseRequests = async () => {
      try {
        const response = await axios.get(`http://localhost:8888/api/admin/${data}/approve/course`,{withCredentials: true});
  ...
  const handleAccept = async (id, info) => {
    ...
    try {
      const response = await axios.post(`http://localhost:8888/api/admin/${data}/approve/course/${id}`, {
        Isapproved: true,
        email: info.Email,
        Firstname: info.enrolledteacher,
      }, {
