...
function VarifyDoc() {
    ...
    const Approval = async(id, type, approve, email)=>{
        try {
          ...
          const response = await fetch(`http://localhost:8888/api/admin/${adminID}/approve/${type}/${id}`, {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        const getData = async () => {
            try {
                ...
                console.log(`http://localhost:8888/api/admin/${adminID}/documents/${type}/${ID}`);
                

                const docData = await fetch(`http://localhost:8888/api/admin/${adminID}/documents/${type}/${ID}`,{
                    method: 'GET',
                    headers: {
                        "Content-Type": "application/json",
                    },
