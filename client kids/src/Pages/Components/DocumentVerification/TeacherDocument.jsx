...
const TeacherDocument = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:8888/api/teacher/TeacherDocument/${Data}`, {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8888/api/teacher/verification/${Data}`, {
        method: "POST",
        body: formDataObj,
         credentials: 'include'
    ...
    <>
      ...
      <div className="flex items-center gap-[20rem] px-32 py-2 bg-[#0D286F]">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl text-[#4E84C1] font-bold">E-learnig For Kids</h1>
        </div>
