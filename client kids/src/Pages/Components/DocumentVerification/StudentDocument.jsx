...
const StudentDocument = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:8888/api/student/StudentDocument/${Data}`, {
          method: "GET",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8888/api/student/verification/${Data}`, {
        method: "POST",
        body: formDataObj,
        credentials: 'include',
      });
