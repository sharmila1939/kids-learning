...
function DashboardTeacher() {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response = await fetch(`http://localhost:8888/api/Teacher/TeacherDocument/${ID}`, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const getAmount = async () => {
      try {
        const response = await fetch(`http://localhost:8888/api/payment/teacher/${ID}/balance`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
