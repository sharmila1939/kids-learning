...
const demoDays = [
  ...
  useEffect(() => {
    const getCourses = async () => {
      try {
        const response = await fetch(`http://localhost:8888/api/course/Teacher/${ID}/enrolled`, {
          method: 'GET',
          headers: {
  ...
  useEffect(() => {
    ...
    console.log(filteredData[0]?.schedule);
  ...
  const addCourses = async () => {
    ...
    if (currentDate > givenDate) {
      ...
      try {
        const response = await fetch(`http://localhost:8888/api/course/${CourseId}/teacher/${ID}/add-class`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      <div className='w-[60%] h-[70%] bg-blue-gray-700 text-white rounded-md'>
        ...
        <div className='flex items-center justify-around my-20 mx-5'>
          <div className='flex gap-5 text-black'>
            ...
            <DateTime setDate={setDate} allowedDays={allowedDays?allowedDays:demoDays}/>
          </div>
        </div>
